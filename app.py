    ...
    .then(geojson => {
      parkingLayer = L.geoJSON(geojson, {
        style: (feature) => ({
          ...
          weight: feature.geometry.type === 'Polygon' ? 2 : 5,
          opacity: 0.85,
        ...
        onEachFeature: (feature, layer) => {
          ...
          layer.bindPopup(`
            <div style="font-family:'Inter',sans-serif; color:#14151a;">
              <strong style="font-size:14px; font-weight:700;">${escapeHtml(props.name)}</strong><br/>
              <span style="font-size:12px; color:#555;">Type: <b>${TYPE_LABEL[props.parking_type] || props.parking_type}</b></span><br/>
...
function logMLFeedback() {
  if (userLat === null || userLon === null) {
    alert('Please allow location access so we can log your GPS position as the parking spot!');
    return;
  }

  const payload = { lat: userLat, lon: userLon };
  if (currentDest) {
    payload.dest_lat = currentDest.lat;
    payload.dest_lon = currentDest.lon;
  }

  fetch('/api/admin/ml_feedback', {
    method: 'POST',
  ...
  .then((res) => {
    let matchMsg = '';
    if (res.parked_at_recommended_spot === true) {
      matchMsg = ' You parked at one of the recommended spots!';
    } else if (res.parked_at_recommended_spot === false) {
      matchMsg = ' You parked somewhere other than the recommended spots.';
    }
    alert('Parking location successfully recorded for ML model training!' + matchMsg);
  });
}
...
function saveAdminFeature() {
  ...
  const payload = {
    ...
    coordinates: [[44.5122, 40.1808], [44.5133, 40.1795]]
  };
